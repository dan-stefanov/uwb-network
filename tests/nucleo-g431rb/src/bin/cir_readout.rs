...
bind_interrupts!(
    struct Irqs {
        ...
        LPUART1 => usart::InterruptHandler<embassy_stm32::peripherals::LPUART1>;
    }
);
...
async fn main(_spawner: Spawner) {
    ...
    let mut uart = unwrap!(usart::Uart::new(
        p.LPUART1,
        p.PA3,
        p.PA2,
        Irqs,
    ...
    if cfg!(feature = "initiator") {
        test_suite::cir_readout::initiator(&mut phy, nucleo_g431rb::UWB_CHANNEL).await;
    } else {
        test_suite::cir_readout::responder(&mut phy, nucleo_g431rb::UWB_CHANNEL, &mut uart).await;
    }
}
